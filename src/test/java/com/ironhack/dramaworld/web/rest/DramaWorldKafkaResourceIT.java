package com.ironhack.dramaworld.web.rest;

import com.ironhack.dramaworld.config.KafkaProperties;
...
class DramaWorldKafkaResourceIT {
    ...
    void setup() {
        ...
        DramaWorldKafkaResource kafkaResource = new DramaWorldKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/drama-world-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/drama-world-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
